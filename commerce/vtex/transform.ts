...
import { LegacyProductVTEX, Product } from "$store/commerce/vtex/types.ts";
...
export const toProduct = <P extends LegacyProductVTEX | ProductVTEX>(
  ...
  const productSpecifications = isLegacyProduct(product)
    ? legacyToProductSpecifications(product)
    : toProductSpecifications(product);
...
const legacyToProductSpecifications = (
  product: LegacyProductVTEX,
) => {
  const properties = product?.allSpecifications?.map((property) => property);

  const specs = properties?.map((property: string) => {
    return {
      name: property,
      value: product?.[property]?.[0] ?? "",
    };
  });
